...
namespace GlobalState {
	const DeviceMemory::Common::HostVisibleConstructArguements fGetHostVisibleMemoryConstructArguements() {
		static DeviceMemory::Common::HostVisibleConstructArguements gConstructArguements{
			.mBufferInfos{ 
				DeviceMemory::Common::BufferInfo(32 * 4, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, fGetDevicesWrapper().mGRAPHICS_QUEUE_FAMILY_INDEX),
				DeviceMemory::Common::BufferInfo(4 * 6, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, fGetDevicesWrapper().mGRAPHICS_QUEUE_FAMILY_INDEX),
	...
	void fPopulateHostVisibleMemory(DeviceMemory::HostVisible& toBePopulated) {
		const std::vector<Vertex::Vertex> VERTICIES{
			Vertex::Vertex(glm::vec4(-0.5f, -0.5f, 0.0f, 1.0f), glm::vec4(1.0f, 0.0f, 0.0f, 1.0f), glm::vec2(0.0f, 0.0f)), // top left
			Vertex::Vertex(glm::vec4(0.5f, -0.5f, 0.0f, 1.0f), glm::vec4(0.0f, 1.0f, 0.0f, 1.0f), glm::vec2(1.0f, 0.0f)), // top right
			Vertex::Vertex(glm::vec4(-0.5f, 0.5f, 0.0f, 1.0f), glm::vec4(0.0f, 0.0f, 1.0f, 1.0f), glm::vec2(0.0f, 1.0f)), // bottom left
			Vertex::Vertex(glm::vec4(0.5f, 0.5f, 0.0f, 1.0f), glm::vec4(0.0f, 1.0f, 0.0f, 1.0f), glm::vec2(1.0f, 1.0f)), // bottom right
		};
	...
	const DeviceMemory::Common::DeviceLocalConstructArguements fGetDeviceLocalMemoryConstructArguements() {
		...
		static DeviceMemory::Common::DeviceLocalConstructArguements gConstructArguements{
			.mBufferInfos{ 
				DeviceMemory::Common::BufferInfo(32 * 4, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, fGetDevicesWrapper().mGRAPHICS_QUEUE_FAMILY_INDEX),
				DeviceMemory::Common::BufferInfo(4 * 6, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, fGetDevicesWrapper().mGRAPHICS_QUEUE_FAMILY_INDEX)
			},
			.mImageInfos{
	...
	void fPopulateDeviceLocalMemory(DeviceMemory::DeviceLocal& toBePopulated) {
		toBePopulated.copyBufferToBuffer(0, fGetHostVisibleMemory().mpBuffers[0], {VkBufferCopy(0, 0, 32 * 4)});
		toBePopulated.copyBufferToBuffer(1, fGetHostVisibleMemory().mpBuffers[1], {VkBufferCopy(0, 0, 4 * 6)});
