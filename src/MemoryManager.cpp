...
namespace MemoryManager {
	void init() {
		...
		g_device_buffers = {};
		g_descriptor_sets = get_descriptor_sets();
	}
	...
	std::vector<DescriptorSet> get_descriptor_sets(const std::vector<DescriptorSet::Write>& writes) {
		std::vector<DescriptorSet> descriptor_sets;
		descriptor_sets.reserve(2); // NOTICE
